...
function add_download_link($which){
    ...
    if($which == 'bottom'){
        ...
        if ( is_post_type_archive('request') ) {
                ...
                $download_link = get_home_url() . '/api/v1/?requests';
...
function create_schoolio_post_types() {
    ...
    register_post_type('ecole', // Register Custom Post Type
    array(
        'labels' => array(
    ...
    register_post_type('request', // Register Custom Post Type
    array(
        'labels' => array(
...
function get_teacher_course_ids($meta){
    ...
    $course_ids = array_map( function($c){ return $c->post_id;   }, $meta);
...
function get_teacher_course_times( $meta ){
    ...
    $course_ids = array_map( function($c){ return $c->post_id;   }, $meta);
    $metakeys_course = array_map( function($c){ return $c->meta_key;   }, $meta);
    $metakeys_time = array_map( function($c){ return  str_replace('teacher', 'time', $c);    }, $metakeys_course);
