	...
	$(function () {
		...
		function showSchoolSpecificFields(school) {
			  ...
			  $('.group_for_cpmdt_courses').hide();
						...
						if (school == 'CMG') {
							var shtml = '<p ><a target="_blank" href="http://www.cmg.ch/sites/default/files/cmusge/public/formulaire_inscription_musique_2017_2018.pdf" class="button">S’inscrire sur le site du CMG</a></p>';
								$('.group_for_non_cpmdt_courses').show().html(shtml);
						} else {
							var shtml = '<p ><a target="_blank" href="http://www.dalcroze.ch/inscription/" class="button">S’inscrire sur le site de l’IJD</a></p>';
							  $('.group_for_non_cpmdt_courses').show().html(shtml);
						}
		...
		function getBadFields(){
			...
			$('.group_visible').each(function(){
					...
					if($radiolength > 0) {
						...
						var $textfields = $("input[type='text'], input[type='email']", $group );
			...
			makeNiceSelectBoxes();
					...
					$( 'select', $field_for_select).each(function(){
						...
						if ( $this.hasClass('dont_show')) {
					 	...
					 	$parent.append('<div class="nicer_sel_container"><ul class="nicer_select_options"></ul><div class="nicer_currently_selected">...</div></div>');
					 	...
					 	for (var i = 0; i < $options.length; i++) {
					 		...
					 		// if ($option.value != '') {	}
					 			$nso.append('<li class="nicer_option" data-val="' + $option.value  + '">'+ $option.text +'</li>');

					 	}


					 	$('label, .nicer_currently_selected',  $parent).on('click', function(e){
					 	...
					 	$this.on('focus', function(e){
					 		...
					 		$('.nicer_select_options', $parent).hide();
					 		$('.nicer_currently_selected', $parent).html($this.find("option:selected").text() );
					 	});
