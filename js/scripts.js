(function ($, root, undefined) {
	...
	$(function () {
		...
		$.featherlight.defaults.afterOpen = function (event) {
			...
			if ($class !== undefined) {
				$('.featherlight-content').attr('class', 'featherlight-content ' + $class);
			}
		...
		if (typeof zone_locations != 'undefined') {
			...
			var zone_map = new google.maps.Map(zone_map_container.get(0), map_options);
			...
			for (var i = 0; i < zone_locations.length; i++) {
				...
				if (zone_location != null) {
					addPointToMap(zone_map, zone_location, zone_bounds, zone_infowindow, zone_markers);
				}
		...
		function displayCourses(courses, courses_container, compiled) {
			...
			if ($search_val != '' || $school.length > 0 || $location.length > 0 || $cat.length > 0) {
